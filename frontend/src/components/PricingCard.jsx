    ...
    <div
      className={`rounded-2xl p-8 flex flex-col ${
        tier.featured
          ...
          : 'bg-white text-charcoal border border-cream-line'
      }`}
    >
      {tier.featured && (
      ...
      <p className={`text-xs mb-5 ${tier.featured ? 'text-cream/60' : 'text-charcoal/50'}`}>{tier.best}</p>
      <div className="mb-6">
        <span className="text-sm align-top mr-1">PKR</span>
        <span className="font-display font-bold text-3xl">{tier.range}</span>
      </div>
